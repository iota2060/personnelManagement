...
//		logger.debug("dao >>> "+map);
	...
	public List<HashMap<String,Object>> vacationCountAutoCalculation(HashMap<String,Object> map) {
		...
		for(String empEmno : words) {
			...
			////////////휴가일수 자동 계산 로직/////////////
			//근로기준법 상에 따른 휴가일수 로직
			//1년간 80% 미만 출근자는 1개월 개근시 1일의 연차 생성
			//==>근태공통코드 W7(결근)이 아닌 날만 한달치 개수 비교하여 개수가 똑같거나 큰 날만큼 연차 생성
	        //최초 1년과 2년은 연차 일수 ‘15일’
	        //3년 이후 2년 단위로 1일씩 가산
			//참고 : matchlive.tistory.com/142
			//////////////////////////////////////
			if(cowyCnt == 0){ //1년 미만
				for(int i=0; i<=monthSize; i++) {
					...
					if(cowyMonthCnt == 1) { //1개월 개근했으면                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           
						vacCnt++; //휴가개수+1
//						logger.info("휴가개수+1:::"+empIncoYearMonth);
					}
...
//					    logger.info("empIncoYearMonth::: "+empIncoYearMonth);
			...
			}else if(cowyCnt >= 1 && cowyYn == 0){ //1년 이상이고 전년도 80% 미만 출근(정상근무 개수 80% 미만)					empIncoYearMonth = Integer.parseInt(LocalDateTime.now().minusYears(1).format(DateTimeFormatter.ofPattern("YYYY"))+"01"); //작년 1월
				for(int i=1; i<=12; i++) {
					map.put("cowyMonthCnt", empIncoYearMonth);
					cowyMonthCnt = this.sqlSession.selectOne(nameSpaceName + "cowyMonthCnt", map);
			...
			}else if(cowyCnt == 1 && cowyYn != 0){ //1년이고 전년도 80%이상 출근
...
//				logger.info(">>>1년<<< 휴가:"+vacCnt);
			}else if(cowyCnt >= 2 && cowyYn != 0) { //2년이상 전년도 80%이상 출근
//				map.put("cowyCnt", cowyCnt);
